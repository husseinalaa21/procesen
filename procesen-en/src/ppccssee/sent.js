        ...
        if (swp.length > 0 && this.state.isW === true) {
            ...
            fetch("https://server.procesen.com/pc/procsentEn/sen/?pr=" + JSON.stringify(daBot))
                ...
                .then((re) => {
                    ...
                    this.iValCom({ a: swp.length, v: "", vb: " There appears to be a problem with the server [002]. Please wait a while and try again . ", n: 0, p: 0, vbPlus : ""})
                })
        } else {
            this.iValCom({ a: swp.length, v: "", vb: "<i class='nullTextArea'> There is nothing yet .. </i>", n: 0, p: 0 ,vbPlus : ""})
        }
    }
    iValCom(x) {
