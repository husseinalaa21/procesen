        ...
        if (mesq.value.length > 0 && usew.value.length > 0 && emai.value.length > 0) {
            ...
            ett.addEventListener("load", e => {
                ...
                <div class="rootGreen">
                    ...
                    <div class="rootMess"> تم الارسال بنجاح . </div>             
                </div>`
                aredSen = true
            });
            ...
            messageBoot.innerHTML = `<div class="messageGre redM"> يرجى التحقق من كتابة المعلومات بشكل صحيح </div>`
            e.preventDefault()
        }
    } else {
        if (aredSen === false) {
            messageBoot.innerHTML = `<div class="messageGre greM"> يرجى أنتضار الارسال .. </div>`
        } else {
            idRoot.innerHTML = `
            <div class="rootGreen">
                <div class="rootLoogo"> <i class="material-icons" style="font-size : 30px;color:#3a6351; text-align: center">done_all</i> </div>
