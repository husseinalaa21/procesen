    ...
    function FormSend() {
        ...
        return (
            <div className={them ? "formSendNotesMain sendReDr" : "formSendNotesMain senReLi"} style={{ textAlign: 'center' }}>
                <p style={them ? textTitleDr : textTitleLi}> أرسال أبلاغ </p>
                <div className="enterNam">
                    <input type="text" className={them ? "inputInf inputInfDr" : "inputInf"} placeholder=" ادخل اسمك هنا " onChange={(x) => textNma = x.target.value}></input>
                </div>
                <div className="enterGmail" >
                    <input type="text" className={them ? "inputInf inputInfDr" : "inputInf"} placeholder=" ادخل ايميلك هنا " onChange={(x) => textEm = x.target.value}></input>
                </div>
                <div className="enterNotes">
                    <textarea onChange={(x) => textArea = x.target.value} className={them ? "textarea textareaDr" : "textarea"} style={{ borderRadius: '3px', color: '#393e46', resize: 'none', backgroundColor: 'rgba(255, 255, 255, 0.527)', borderRradius: '3px', width: '90%', border: '0', padding: '10px' }} rows="10" placeholder=" ادخل ملاحظتك هنا "></textarea>
                </div>
                <div className="entserSendThin " style={them ? btnDr : btnLi} onClick={() => { subm() }}> أرسال </div>
            </div>
        )
    }

    function ThenSendThis(x) {
        var thmFormLi = isCancel ? { padding: '5px', paddingTop: '0' } : { padding: '10px', border: '0', borderRight: ' 4px solid  #2682df', width: '80%', borderRadius: '2px', margin: 'auto', backgroundColor: ' rgb(245, 245, 245)', marginTop: '20px' }
        var thmFormDr = isCancel ? { padding: '5px', paddingTop: '0' } : { padding: '10px', border: '0', borderRight: ' 4px solid  #2682df', width: '80%', borderRadius: '2px', margin: 'auto', backgroundColor: 'rgb(15, 34, 70)', marginTop: '20px' }
        if (isSendIt === true) {
            return (
                <div> شكراً لمساهمتك . </div>
            )
        } else {
            return (
                <>
