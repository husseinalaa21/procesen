...
export default class Sent extends Component {
    ...
    adAdvSet(x, y, u) {
        ...
        if(x.includes(this.lsBlack[0]) === true || x.includes(this.lsBlack[1]) === true || y.includes(this.lsBlack[0]) === true || y.includes(this.lsBlack[1]) === true){
            this.setState({mesAdsList : <div className="wrongMessLis"> <FontAwesomeIcon icon={faTools} />  &nbsp; واحدة من القيم المدخلة أو كلتيهما غير صالحة. قد تكون القيمة التي تحاول إدخالها موجودة بالفعل في خيارات النظام أعلاه .</div>})
            return false
        }
        if (this.state.isclc === true) {
            if (x.length > 0 && y.length > 0) {
                ...
                if (ise === false) {
                    ...
                    this.setState({mesAdsList : <div className="wrongMessLis"> <FontAwesomeIcon icon={faInfoCircle}/> &nbsp; يبدو أن القيمة أو الاسم قيد الاستخدام بالفعل. الرجاء اختيار اسم جديد أو قيمة جديدة صالحة .  </div>})
                }
            }
        } else {
            if (x.length > 0) {
